...
const summarizeClassRoom = (studentList: Student[]): NewStudent[] => {
  let newobj: NewStudent[] = [];
  studentList.map((student, index) => {
    const nombre = studentList[index].name;
    const alta = Math.max(...studentList[index].califications);
    const media = (
      studentList[index].califications.reduce((a, b) => a + b) /
      studentList[index].califications.length
    ).toFixed(2);

    let studiante: NewStudent = {
      name: nombre,
      highestCalification: alta,
      averageCalifications: media
    };
    newobj = [...newobj, studiante];
  });
  return newobj;
};
