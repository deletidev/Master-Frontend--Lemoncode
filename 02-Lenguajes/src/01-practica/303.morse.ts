...
interface TiemposEspera {
  punto: number;
  raya: number;
  espaciosLetras: number;
  espaciosPalabras: number;
  base: number;
}

const tiemposEspera: TiemposEspera = {
  punto: 1,
  raya: 3,
  espaciosLetras: 1,
  espaciosPalabras: 7,
...
class transmisorMorse {
  private espera: number;
  private mostrarEstado: (estado: Estado) => void;
  private traducirCaracter: (caracter: string) => string[];
  private tiempo: (arg1: Estado, arg2: number) => void;

  constructor() {
    this.espera = 0;

    this.mostrarEstado = estado =>
      estado === 'On'
        ? console.log(`%c${estado}`, 'color: green; font-size:18px')
        : console.log(`%c${estado}`, 'color: darkred; font-size:14px');

    this.traducirCaracter = caracter => morseAlphabet[caracter].split('');

    this.tiempo = (estado, ptos) =>
      setTimeout(() => {
        this.mostrarEstado(estado);
      }, ptos * tiemposEspera.base);
  }

  traductor(mensajes: string) {
    const dividir = mensajes.toLowerCase().split('');
    dividir.forEach(caracter => {
      if (caracter === ' ') {
        this.espera +=
          tiemposEspera.espaciosPalabras - tiemposEspera.espaciosLetras;
      } else if (morseAlphabet.hasOwnProperty(caracter)) {
        const simbolos = this.traducirCaracter(caracter);
        simbolos.forEach((simbol: string) => {
          if (simbol === '.') {
            this.tiempo('On', this.espera);
            this.espera += tiemposEspera.punto;
            this.tiempo('Off', this.espera);
          }
          if (simbol === '-') {
            this.tiempo('On', this.espera);
            this.espera += tiemposEspera.raya;
            this.tiempo('Off', this.espera);
          }
          this.espera += tiemposEspera.espaciosLetras;
        });
      }
    });
  }
...
const nuevoTransmisor = new transmisorMorse();
const mensajeMorse = 'Funciona';
nuevoTransmisor.traductor(mensajeMorse);
